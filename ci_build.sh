...
default|default-Werror|default-with-docs|valgrind|clang-format-check)
    ...
    ; then
        ...
        cd ./tmp-deps
        $CI_TIME git clone --quiet --depth 1 -b release/IPM_Infra-1.5.0 https://github.com/42ity/libcidr.git cidr
        cd ./cidr
        CCACHE_BASEDIR=${PWD}
        export CCACHE_BASEDIR
        git --no-pager log --oneline -n1
    ...
    ; then
        ...
        cd ./tmp-deps
        $CI_TIME git clone --quiet --depth 1 -b release/IPM_Infra-1.5.0 https://github.com/42ity/nut.git libnutclient
        cd ./libnutclient
        CCACHE_BASEDIR=${PWD}
        export CCACHE_BASEDIR
        git --no-pager log --oneline -n1
