/*  =========================================================================
    fty_nut_discovery_server - Manages discovery requests, provides feedback
...
struct _fty_nut_discovery_server_t {
    mlm_client_t *mlm;
...
zactor_t* range_scanner_new(fty_nut_discovery_server_t *self) {
...
void reset_nb_discovered(fty_nut_discovery_server_t *self) {
...
void configure_local_scan(fty_nut_discovery_server_t *self) {
    int s, family, prefix;
...
bool compute_configuration_file(fty_nut_discovery_server_t *self) {
...
void
ftydiscovery_create_asset(fty_nut_discovery_server_t *self, zmsg_t **msg_p) {
...
bool static
s_handle_pipe(fty_nut_discovery_server_t* self, zmsg_t *message, zpoller_t *poller) {
    if (!message)
        return true;
...
void static
s_handle_mailbox(fty_nut_discovery_server_t* self, zmsg_t *msg, zpoller_t *poller) {
    if (is_fty_proto(msg)) {
...
void static
s_handle_stream(fty_nut_discovery_server_t* self, zmsg_t *message) {
...
void static
s_handle_range_scanner(fty_nut_discovery_server_t* self,
...
//  --------------------------------------------------------------------------
//  fty_nut_discovery_server actor

void
fty_nut_discovery_server(zsock_t *pipe, void *args) {
    fty_nut_discovery_server_t *self = fty_nut_discovery_server_new();
    ...
    fty_nut_discovery_server_destroy(&self);
    zpoller_destroy(&poller);
}

//  --------------------------------------------------------------------------
//  Create a new fty_nut_discovery_server

fty_nut_discovery_server_t *
fty_nut_discovery_server_new() {
    fty_nut_discovery_server_t *self = (fty_nut_discovery_server_t *) zmalloc(sizeof (fty_nut_discovery_server_t));
    assert(self);
...
void
fty_nut_discovery_server_destroy(fty_nut_discovery_server_t **self_p) {
    assert(self_p);
    if (*self_p) {
        fty_nut_discovery_server_t *self = *self_p;
...
void
fty_nut_discovery_server_test(bool verbose) {
    printf(" * ftydiscovery: ");

    //  @selftest
    //  Simple create/destroy test
    zactor_t *self = zactor_new(fty_nut_discovery_server, NULL);
    assert(self);
    zclock_sleep(500);
    zactor_destroy(&self);
    //  @end
