...
; then
 ...
 if [ -d "$FOLDER_NAME" ]; then
    ...
    if [ "x$REQUESTED_BRANCH" = "x" ]; then
        echo "git clone https://github.com/42ity/libcidr.git $FOLDER_NAME"
        $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/libcidr.git $FOLDER_NAME
    else
        if git ls-remote --heads https://github.com/42ity/libcidr.git | grep -q "$REQUESTED_BRANCH"; then
            echo "git clone -b "$REQUESTED_BRANCH" https://github.com/42ity/libcidr.git $FOLDER_NAME"
            $CI_TIME git clone --quiet --depth 1 -b "$REQUESTED_BRANCH" https://github.com/42ity/libcidr.git $FOLDER_NAME
        else
            echo "$REQUESTED_BRANCH not found for https://github.com/42ity/libcidr.git"
            echo "git clone https://github.com/42ity/libcidr.git $FOLDER_NAME"
            $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/libcidr.git $FOLDER_NAME
        fi
    fi
...
; then
 ...
 if [ -d "$FOLDER_NAME" ]; then
    ...
    if [ "x$REQUESTED_BRANCH" = "x" ]; then
        echo "git clone https://github.com/42ity/nut.git $FOLDER_NAME"
        $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/nut.git $FOLDER_NAME
    else
        if git ls-remote --heads https://github.com/42ity/nut.git | grep -q "$REQUESTED_BRANCH"; then
            echo "git clone -b "$REQUESTED_BRANCH" https://github.com/42ity/nut.git $FOLDER_NAME"
            $CI_TIME git clone --quiet --depth 1 -b "$REQUESTED_BRANCH" https://github.com/42ity/nut.git $FOLDER_NAME
        else
            echo "$REQUESTED_BRANCH not found for https://github.com/42ity/nut.git"
            echo "git clone https://github.com/42ity/nut.git $FOLDER_NAME"
            $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/nut.git $FOLDER_NAME
        fi
    fi
    ...
    if [ -e ci_dependencies.sh ]; then
        ...
        DEFAULT_BRANCH="`git symbolic-ref refs/remotes/origin/HEAD | sed 's@^refs/remotes/origin/@@'`"
        if [ "x$REQUESTED_BRANCH" = "x" ]; then
            echo "`date`: INFO: Building prerequisites of 'libnutclient' using ci_dependencies.sh the default branch..." >&2
            ($CI_TIME source ./ci_dependencies.sh)
        elif [ "x$PROPAGATED_BRANCH" = "x$DEFAULT_BRANCH" ]; then
            echo "`date`: INFO: Building prerequisites of 'libnutclient' using ci_dependencies.sh the default branch..." >&2
            ($CI_TIME source ./ci_dependencies.sh)
        else
            echo "`date`: INFO: Building prerequisites of 'libnutclient' using ci_dependencies.sh $PROPAGATED_BRANCH branch..." >&2
            ($CI_TIME source ./ci_dependencies.sh $PROPAGATED_BRANCH)
        fi
    fi
    if [ -e autogen.sh ]; then
...
if ! (command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libfty_common_db-dev >/dev/null 2>&1) || \
       ...
       ([ -e "fty-common-db" ]) \
; then
 FOLDER_NAME="fty-common-db-master"
 ...
 if [ -d "$FOLDER_NAME" ]; then
    ...
    echo "git clone -b master https://github.com/42ity/fty-common-db.git $FOLDER_NAME"
    $CI_TIME git clone --quiet --depth 1 -b master https://github.com/42ity/fty-common-db.git $FOLDER_NAME
    cd "./$FOLDER_NAME"
    CCACHE_BASEDIR=${PWD}
    export CCACHE_BASEDIR
        git --no-pager log --oneline -n1
