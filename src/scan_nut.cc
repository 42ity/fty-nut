...
scan_nut_actor(zsock_t *pipe, void *args)
{
    ...
    int nut_scanner_timeout = std::stoi(DEFAULT_NUTSCAN_TIMEOUT);
    zconfig_t *config = zconfig_load(getDiscoveryConfigFile().c_str());
    if (config) {
        nut_scanner_timeout = std::stoi(zconfig_get(config, CFG_PARAM_NUTSCAN_TIMEOUT, DEFAULT_NUTSCAN_TIMEOUT));
        zconfig_destroy(&config);
    }
    else {
        log_error("Failed to load config file %s", getDiscoveryConfigFile().c_str());
    ...
    if(!stop_now) {
        for(const auto& it : credentialsSNMPv1) {
            std::vector<std::string> output;
            nut_scan_snmpv1 ("device", listAddr->firstAddress(), listAddr->lastAddress(), it, false, nut_scanner_timeout, output);
            outputs.push_back(std::make_pair(output, it.community));
    ...
    if(!zsys_interrupted && !stop_now) {
        nut_scan_xml_http ("device", listAddr->firstAddress(), listAddr->lastAddress(), nut_scanner_timeout, outputXml);
    }
