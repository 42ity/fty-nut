...
public:
    static void test()
    {
        ...
        assert(reader1->getState().empty());
        assert(reader2->getState().empty());

        Asset *a = new Asset;
        a->property = "test1";
        writer.getState().emplace("asset1", std::shared_ptr<Asset>(a));
        // Not yet committed
        ...
        assert(reader1->getState().size() == 1);
        assert(reader1->getState().count("asset1") == 1);
        // reader2 did not update yet
        assert(reader2->getState().empty());

        a = new Asset;
        a->property = "test2";
        writer.getState().emplace("asset2", std::shared_ptr<Asset>(a));
        writer.commit();
        assert(manager.states_.size() == 3);
        assert(reader1->refresh());
        assert(reader1->getState().size() == 2);
        assert(reader1->getState().count("asset1") == 1);
        assert(reader1->getState().count("asset2") == 1);
        // reader2 is two steps behind
        assert(reader2->getState().empty());
        assert(reader2->refresh());
        assert(reader2->getState().size() == 2);
        assert(reader2->getState().count("asset1") == 1);
        assert(reader2->getState().count("asset2") == 1);

        // Force a cleanup and check that we discarded the two old states
        manager.cleanup();
        assert(manager.states_.size() == 1);

        writer.getState().erase("asset1");
        writer.commit();
    }
};
